...
## 결정 5 — 도판 판정은 이 서버의 GPU로 (D118)

교과서 도판 캡션 판정(`figure_judge.py`)은 OpenAI 호환 비전 엔드포인트면 무엇이든
꽂을 수 있다. 이 서버에 A100 80GB가 두 장 놀고 있어서 llama.cpp로 EXAONE-4.5-33B를
직접 띄운다 — 외부 API 비용도, 학생 자료가 밖으로 나갈 일도 없다.

**원래 이건 사람이 손으로 띄운 프로세스였다.** supervisor 밖이라 상태에 안 잡히고,
죽어도 아무도 모르고, 되살리는 방법이 어디에도 없었다. 게다가 `--host 0.0.0.0`에
인증이 없어서 파드 네트워크에 그냥 열려 있었다. 이번에 셋 다 고쳤다:
supervisor 편입 · `127.0.0.1` 바인딩 · `--api-key-file`.

가중치 22GB의 위치가 이 서버에서 유일하게 영속/휘발 규칙을 벗어난다:

| | 위치 | 왜 |
|---|---|---|
| 실행 | `~/models/exaone45/` (오버레이) | NFS mmap은 기동이 느리다 |
| 아카이브 | `~/data/nodi/models/` (NFS) | 오버레이가 날아가도 40초면 복구(실측 580MB/s) |

`bootstrap.sh`가 둘 중 있는 쪽에서 없는 쪽으로 복사한다. 둘 다 없으면 판정만
끄고 넘어간다 — **22GB 다운로드가 배포를 붙잡게 두지 않는다.**

판정이 꺼져도 교과서 업로드는 정상이다(D103: 파서가 `caption`/`footnote`로 라벨한
도판은 판정 없이 처리된다). 라벨 없는 도판만 캡션 없이 실패한다.
