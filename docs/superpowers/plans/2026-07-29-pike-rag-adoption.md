...
**Goal:** PIKE-RAG의 검색 품질 기법 4종(지식 원자화 이중 검색 D116, 태스크 분해 강화 D117, figure 캡션 비전 생성 D118, LLM 의미 청킹 D119)을 킬스위치 기본 off로 이식한다.
...
- `common.touch_job(svc, job_id: str) -> None` — jobs.updated_at 전진(하트비트, D120)
...
- [ ] **Step 10: 커밋** — `[feat]: PIKE-RAG 기반 — chunk_atoms 스키마·노브·컬렉션·잡 하트비트 (D116/D120)` (자기 파일만 add)
...
```python
"""원자 질문 생성 프롬프트·파서 (TASK 6, D116) — PIKE-RAG atom_prompt_ko 이식.
...
- [ ] **Step 5: 커밋** — `[feat]: 원자 질문 프롬프트·파서 — PIKE atom_prompt_ko 이식 (D116)`
...
- [ ] **Step 6: 커밋** — `[feat]: atom_batch 잡 — 청크당 예상 질문 생성·chunk_atoms 적재 (D116)`
...
3. **게이트 분리**: 직접 히트는 `class_material_rag_max_distance`(0.60), 원자 히트는 `atom_rag_max_distance`(0.45)를 **원자 거리**(1-score)에 적용. 원자 경유 청크에 청크 게이트를 재적용하지 않는다(D116 — "청크 벡터로는 멀지만 원자 질문으로는 정확한" 케이스가 원자화의 목적).
...
- [ ] **Step 2: FAIL 확인** → **Step 3: rag.py에 dual_search 구현** (search() 아래, 독스트링에 D116·게이트 분리 근거) → **Step 4: PASS + 전체 스위트**
- [ ] **Step 5: 커밋** — `[feat]: rag.dual_search — 청크·원자 이중 검색과 병합 게이트 (D116)`
...
- [ ] **Step 5: PASS + 전체 스위트** → **Step 6: 커밋** — `[feat]: 자료 검색 스킬 이중 검색 배선 + 삭제 시 원자 퍼지 (D116)`
...
- [ ] **Step 5: PASS + 전체 스위트** → **Step 6: 커밋** — `[feat]: ReAct 분해 지침 + 검색어 정제 노브 (D117)`
...
- [ ] **Step 4: PASS** → **Step 5: 커밋** — `[feat]: figure 캡션 비전 생성 클라이언트 (D118)`
...
- `_handle_figure_batch`: select에 `page_text` 컬럼 추가. **분기**:
  ...
  - on → parsed/판정 분류 대신 **전 행을 생성 대상으로**: `figure_judge.is_configured()` 확인(미설정이면 전 행 `match_kind='no-caption'` failed — 기존 미설정 처리와 동형), 이미지 다운로드 후 `figure_caption.caption_all(items, concurrency=…, heartbeat=lambda: common.touch_job(svc, job["id"]))`. 행별:
    ...
    - 실패 + `row["caption"]`(parsed) 존재 → patch `match_kind: "parsed"`, embed_text=parsed (D118 폴백)
    - 실패 + parsed 없음 → 행 failed `match_kind='caption-error'`
  - 이후 임베딩→Qdrant→`_mark`는 기존 코드 공용(무수정).
- 모듈 독스트링 갱신: D118 결정(생성 일원화, parsed는 프롬프트 입력·폴백으로 격하) 명시.
...
- [ ] **Step 5: 커밋** — `[feat]: figure 캡션 비전 생성 경로 — 페이지 텍스트 영속·parsed 폴백 (D118)`
...
- [ ] **Step 2: FAIL 확인** → **Step 3: 구현**(모듈 독스트링에 D119·PIKE 이식·v1 요약 생략 근거) → **Step 4: PASS**
- [ ] **Step 5: 커밋** — `[feat]: LLM 의미 청킹 — resplit 경계 재조정 (D119)`
...
```python
    ...
    if sem_on and not is_session_upload and len(text) <= sem_max:
        # D119: LLM 의미 청킹 — 어떤 실패든 정규식 폴백(인덱싱 불가침).
        try:
...
- [ ] **Step 5: 커밋** — `[feat]: split 의미 청킹 분기 — 크기 가드·정규식 폴백 (D119)`
...
- [ ] CLAUDE.md 갱신: 핵심 파이프라인 절에 D116~D120 반영(원자화·이중 검색·캡션 생성·의미 청킹·하트비트), "비전 판정 미구현" 서술 현행화
