...
ReAct로 뜬 도판을 캔버스에서 다룰 수 있게 한다:

- **이동** — 드래그로 옮기고, 옮긴 자리는 새로고침 후에도 유지된다.
- **리사이즈** — 여덟 손잡이로 크기 조절. **가로세로 비율 고정**(이미지가 찌그러지지
  않는다). 조절한 크기도 새로고침 후 유지된다. **크기 조절은 도판만 가능하다**
  (사용자 지시 2026-08-02 — 글 상자는 크기 조절을 없앤다, 아래 참조).
- **삭제** — × 버튼 또는 키보드 Delete/Backspace.
- **선택** — 클릭·올가미·그룹 선택(글 상자와 섞어서). 선택 시 테두리 표시.

이동·삭제·선택은 글 상자(TextItem)와 동등하지만, **리사이즈는 도판 전용**이다.
...
- **도판 편집·태그·인출 연습** — 도판엔 본문이 없다. ItemMenu의 편집/태그/인출은
  붙이지 않는다.
- **글 상자(TextItem) 리사이즈 — 제거한다(D142 되돌림).** 사용자 지시(2026-08-02)로
  크기 조절은 **도판만** 가능하다. TextItem의 `ResizeHandles`·`data.size` 사용·
  ItemMenu의 "크기 되돌리기"를 걷어낸다. 글 상자의 이동·선택·편집·삭제는 유지한다.
...
- `aspect`가 없으면 현행 자유 w/h 로직 그대로(향후 재사용 여지로 남겨 둔다 —
  현재 소비자는 도판뿐이고 늘 `aspect`를 준다).
- FigureItem은 이미지의 **자연 비율**(`img.naturalWidth/naturalHeight`, 로드 후
  확보. 아직이면 현재 렌더 상자 비율)을 넘긴다.

**TextItem에서 `ResizeHandles`를 걷어낸다.** 사용자 지시(2026-08-02)로 글 상자는
크기를 조절하지 않는다. `data.size`(D142) 필드는 이제 **도판 전용**이 된다 —
타입 주석을 그에 맞춰 고친다. `onResize`/`onResetSize` 핸들러와 크기 관련
되돌리기(`undoLabel`의 size 분기)는 도판이 쓰므로 그대로 둔다.
