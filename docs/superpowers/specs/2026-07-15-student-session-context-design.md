...
- **첨부 진입점**: BottomBar 바로 위에 부착되는 자립 컴포넌트
  `SessionFilesBar`(클립 버튼 + 파일 칩 스트립, 계획 시점 정제 — BottomBar
  내부 개조 대신 상태를 한 곳에 응집) — 현재 활성 세션으로 업로드.
  `uploadFile`에 `session_id` 옵션 추가, 공간 인자는 세션의
  `space_kind/space_ref`를 그대로 전달. 개인 세션·학급 세션 모두 동일 동선.
