...
- [ ] 진행 중 (2026-07-29 착수 — 스펙 승인 완료)

**범위**: Microsoft PIKE-RAG(ICML 2025)의 검색 품질 기법 4종을 단계 구현한다 —
A) 지식 원자화 + 이중 검색(D116): 청크당 solar-pro2 예상 질문 생성 → Qdrant
`chunk_atoms` 컬렉션 → 청크·원자 이중 검색. B→C) 태스크 분해 강화(D117): ReAct 판단
프롬프트 분해 지침 + 스킬 내 질문 정제(기본 off). D) figure 캡션 비전 생성(D118):
페이지 텍스트를 컨텍스트로 비전 모델이 캡션 생성 — D93/D103 후보 선택 대체.
B) LLM 의미 청킹(D119): PIKE resplit 이식. 공통: 잡 하트비트 `touch_job`(D120).
