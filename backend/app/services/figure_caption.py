...
처리한다(폴백 없음, D134). 개별 실패는 raise하지 않고 None으로 강등한다.
"""
...
async def caption_all(
    ...
    *,
    ...
    heartbeat가 있으면 await한다(장기 잡 하트비트 D133 — jobs.updated_at 전진으로
    스테일 재클레임 오탐 방지).

    개별 실패(재시도 포함 최종 예외)·빈 캡션은 raise하지 않고 None을 반환한다 —
    호출부(워커)가 해당 행을 caption-error failed로 처리한다(폴백 없음, D134).
