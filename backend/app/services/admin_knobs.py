...
_SPECS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "scope": "live",
        "description": (
            "한 답에 곁들일 도판 수 상한(거리 게이트를 통과한 것 중). "
            "기본 1 — 곁들이는 답을 **거드는 것**이지 답과 나란히 설 것이 "
            "아니라, 여러 장이 붙으면 카드 옆에 그림 기둥이 선다. "
            "상한 5는 **시험용**이다(사용자 지시 2026-08-12)."
        ),
        "effect": "도판 노출량",
    },
    {
        ...
        "scope": "live",
        "description": (
            "개념 카드 하나에 곁들일 강의 클립 수 상한 (D190: 기본 1). "
            "상한 5는 **시험용**이다(사용자 지시 2026-08-12) — 실제 수업에서는 "
            "1을 그대로 쓰는 것을 전제로 한다."
        ),
        "effect": "클립 노출량",
    },
    ...
    # ── 홈 개념 지도 (사용자 지시 2026-08-10) ────────────────────────
    {
        "key": "home_drift_force",
        "label": "지도 흔들림 폭",
        "group": "홈 지도",
        "widget": "slider",
        "min": 0.0, "max": 2.0, "step": 0.05, "unit": "",
        "scope": "live",
        "description": (
            "홈 배경 지도의 노드가 이웃 쪽으로 밀고 당기는 힘. 0이면 멈춘 것과 "
            "같고, 올릴수록 크게 흔들린다. **폭**만 정한다 — 속도는 아래 값이다."
        ),
        "effect": "흔들리는 폭",
    },
    {
        "key": "home_drift_breath",
        "label": "지도 흔들림 속도",
        "group": "홈 지도",
        "widget": "slider",
        "min": 0.001, "max": 0.05, "step": 0.001, "unit": "rad/틱",
        "scope": "live",
        "description": (
            "한 번 밀고 당기는 데 걸리는 위상 진행. 0.006이면 60fps에서 한 "
            "호흡이 약 17초다. 크면 떨림으로 보이고 작으면 멈춘 것과 구별이 "
            "안 된다. \"천천히\"는 여기를 내려서 얻는다."
        ),
        "effect": "호흡 주기",
    },
    {
        "key": "home_drift_anchor",
        "label": "지도 제자리 힘",
        "group": "홈 지도",
        "widget": "slider",
        ...
        "scope": "live",
        "description": (
            "노드를 처음 자리로 당기는 힘. 흔들리는 폭은 위의 '흔들림 폭'과 "
            "이 값의 **비**로 정해진다. 너무 낮추면 무리가 천천히 흘러가 "
            "지도가 아니게 된다(실측: 30초에 430 → 312로 오그라들었다)."
        ),
        "effect": "폭 ↔ 자리 안정",
    },
    {
        ...
        "group": "홈 지도",
        "widget": "slider",
        "min": 1.0, "max": 2.5, "step": 0.01, "unit": "배",
        "scope": "live",
        "description": (
            "전체가 들어오게 맞춘 뒤 한 번 더 당기는 배율. 1보다 크면 "
            "가장자리가 넘치는데, 넘친 쪽은 인사말 뒤로 보내므로 잘리지 않는다."
        ),
        "effect": "지도 크기",
    },
    {
