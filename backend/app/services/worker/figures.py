"""figure_batch 잡 — 교과서 도판 크롭 · 캡션 생성 · 임베딩 (D86, D118, D121).

캡션은 **비전 생성 단독**이다(D121, 사용자 결정 2026-07-30): 전 pending 행을
...
async def _handle_figure_batch(svc: ServiceClient, job: dict[str, Any]) -> None:
    ...
    _handle_batch를 본떴다. D121: 캡션은 비전 생성 단독 — 생성 실패(caption-error)
    ...
    # ── D121: 캡션은 비전 생성 단독 — 전 pending 행이 생성 대상이다. parsed
    ...
    if not figure_judge.is_configured():
        ...
        for row, cap in zip(figures, captions, strict=True):
            if cap:
                ...
                # 생성 실패 — 폴백 없이 failed(D121). retry가 재생성한다.
                await svc.update(
                    "textbook_figures", {"id": f"eq.{row['id']}"},
