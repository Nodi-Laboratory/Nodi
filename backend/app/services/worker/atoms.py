"""atom_batch 잡 — 청크당 예상 질문 생성 · 임베딩 · Qdrant 적재 (D116, PIKE-RAG).
...
# ---------------------------------------------------------------------------
# atom_batch (D116 — chunk_atoms 질문 생성·임베딩·Qdrant 적재)
# ---------------------------------------------------------------------------
async def _handle_atom_batch(svc: ServiceClient, job: dict[str, Any]) -> None:
    ...
    async def _gen(chunk: dict[str, Any]) -> None:
        async with sem:
            ...
            # 4청크마다 하트비트 — 스테일 복구 오탐 재클레임 방지(D120).
            progress["count"] += 1
...
async def _requeue_atoms(
    ...
    """D116: 실패한 원자 행을 pending으로 리셋하고 atom_batch를 재팬아웃.
