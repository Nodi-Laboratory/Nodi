"""교과서 figure 추출(D86·D134) — labs extract.py 이식.

좌표는 페이지 기준 0~1 정규화. D134(사용자 결정 2026-07-30): 캡션은 비전
...
D103의 후보 선택 경로는 제거됐다. 여기서 뽑는 것은 생성 프롬프트의 입력이다:
  ...
  - page_texts(페이지 본문): 생성 프롬프트의 컨텍스트(D131).
...
def extract_figures(elements: list[dict]) -> list[dict]:
    """Upstage 파싱 elements → figure 레코드 목록(D134).
    ...
    않는다 — 비전 생성(figure_caption)이 워커에서 확정한다(D134).
    ...
    for page, els in sorted(by_page.items()):
        ...
        for el in els:
            ...
            # D134: 파서가 캡션으로 라벨한 요소가 가까이 있으면 parsed 힌트로
...
def page_texts(elements: list[dict], max_chars: int = 4000) -> dict[int, str]:
    """페이지별 본문 텍스트(D131 캡션 비전 생성 컨텍스트) — {page: text}.
