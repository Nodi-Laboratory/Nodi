...
- **캔버스 v2 (D120~D128)**: 카드를 없애고 **학생이 편집하는 작업 공간**으로
  ...
  · **올가미 판정은 완전 포함**(`rect.contains`). Excalidraw가 그 규칙을 쓰기
    때문이다 — 교차 판정을 쓰면 같은 드래그가 글은 잡고 도형은 놓친다.
    올가미 드래그의 전파를 끊지 않는 것도 같은 이유다(저쪽이 도형을 잡아야 한다).
  · **연결선 앵커는 상대 위치가 정한다**(D126, `lib/canvas2/connector.ts`).
    고정 앵커였을 때 같은 부모의 답 둘이 한 점에서 나가 선이 겹쳤다. 기하는
    컴포넌트가 아니라 lib에 둔다 — 이런 결함은 눈보다 테스트로 잡아야 한다.
