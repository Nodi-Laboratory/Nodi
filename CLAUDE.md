...
- **캔버스 v2 (D120~D128)**: 카드를 없애고 **학생이 편집하는 작업 공간**으로
  ...
  · **연결선은 "AI에게 묻기"에서만 생긴다**(사용자 지시 2026-07-31). 하단
    입력창의 평범한 질문은 상자를 더 만들지 않고, 질문 원문만
    `data.askedQuestion`으로 답에 실어 hover 툴팁이 쓴다.
...
- **학습 효과는 캔버스가 아니라 붙인 활동이 만든다 (D138)**. 조사 결론이다 —
  ...
  그래서 "안 보고 다시 말해보기"(D138)를 붙였다. **본문을 가리고 → 쓰고 →
  저장한 뒤 → 원문을 보인다.** 이 순서가 개입의 핵심이다(먼저 보이면 베끼기).
  다음 후보는 **선응답 게이트** — 답 주기 전에 학생이 먼저 한 줄 쓰게 하기
