...
- 선생님은 별도 버튼으로 **교과서**(`kind='textbook'`, PDF 전용)도 업로드한다
  ...
  (D121, 사용자 결정 2026-07-30 — D93 후보 선택·D103 2단 경로·D118 노브 대체):
...
교과서 도판 비전 캡션 생성(`figure_caption.py`, D118·D121 — `figure_judge.py`는
설정 게이트·공용 유틸만 남음)만 별도 계열 — judge_* 노브(base_url/model/api_key,
OpenAI 호환, 기본 EXAONE-4.5)가 전부 설정돼야 동작한다.
...
- **파일 인제스트** (`services/worker/`, D105로 책임별 분할 —
  ...
  **D116 지식 원자화**(`atom_rag_enabled`, 기본 off): on이면 `atom_batch` 잡이
  ...
  생성(D121 단독 경로 — 이미지 + `page_text` + 파서 라벨 힌트로 생성,
