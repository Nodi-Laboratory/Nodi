...
> 데이터·인증·파일은 전부 로컬에서 돈다. 교과서 도판 비전(`JUDGE_*`)은 선택이지만,
> ⚠️ **비워 두면 도판이 전부 검색에서 사라진다** — 캡션은 비전 생성 단독이고
> 실패에 폴백이 없다(D134). 텍스트 RAG는 정상 동작하므로 로컬 개발에는 지장이
> 없고, 도판을 쓰려면 채워야 한다.
...
- **태그 기반 배치** — 모델이 개념마다 자유 태그(대단원 수준)를 붙이고, 프론트가
  **태그마다 세로 열 하나**를 첫 등장 순서로 고정해 묶는다(D123). 열 안에서는
  대화 트리 순서로 위→아래. **무겹침이 알고리즘의 성질**이지 수렴의 결과가
  아니다 — 힘 배치(d3-force)는 그것을 보장하지 못해 버렸다.
- **한 턴에 카드 하나** — 모델이 개념을 여럿 써도 화면은 카드 하나만 만든다
  (D162). 넘치는 개념은 버리지 않고 그 카드에 이어 붙이며 제목은 굵은 줄로
  남는다. 상한은 `canvas_cards_per_turn`(기본 1, 1~5).
- **온보딩 설문** — 가입 직후 이름·학년·학습 단계·목표를 묻는다(D222). 지금은
  아무 기능도 이 값을 읽지 않으므로 **빈 답도 통과**하고 저장 실패도 온보딩을
  막지 않는다.
...
- **교과서 figure** — 교과서 PDF에서 도판을 추출하고, **비전 모델이 페이지 본문을
  컨텍스트로 캡션을 생성**해(D134) 임베딩한다. 학생 질의와 가까운 도판은 캔버스에
  뜬다.
  > ⚠️ **캡션은 비전 생성 단독이고 폴백이 없다.** `JUDGE_*`가 비어 있으면 도판이
  > **전부** 실패로 남아 검색에서 사라진다(파서 라벨은 생성 프롬프트의 힌트로만
  > 쓰인다). 도판 실패는 텍스트 인덱싱과 격리되므로 자료 RAG는 정상이다.
  > 왜 안 뜨는지는 `/admin` 검색 실험실 맨 위의 **진단 표**가 학급마다 알려 준다
  > — 교과서 수 · 상태별 도판 수 · 켜 둔 강의 패키지 · 비전 설정 여부.
