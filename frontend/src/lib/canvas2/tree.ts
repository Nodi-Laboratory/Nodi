...
/**
 * 이 카드와 **선으로 이어져 있는** 카드 전부(자기 자신 포함) — 방향은 안 본다
 * (D210 6-2).
 *
 * `descendants`는 아래로만 내려가므로 형제·부모가 빠진다. 여기서 필요한 것은
 * "이 묶음이 몇 장짜리인가"라서 위아래를 다 봐야 한다 — 자식이 없는 카드도
 * 부모와 이어져 있으면 혼자가 아니다.
 *
 * 간선은 `treeEdges`와 같은 규칙을 쓴다. 다른 규칙으로 세면 화면에 선이
 * 보이는데 "혼자"라고 판정하는 어긋남이 생기고, 그건 학생 눈에 고장이다.
 */
export function groupSizes(items: readonly TreeItem[]): Map<string, number> {
  const near = new Map<string, string[]>();
  const add = (a: string, b: string) => {
    const cur = near.get(a);
    if (cur) cur.push(b);
    else near.set(a, [b]);
  };
  for (const e of treeEdges(items)) {
    add(e.from, e.to);
    add(e.to, e.from);
  }
  ...
  for (const i of items) {
    ...
    // 덩어리 하나를 통째로 걷고 그 크기를 구성원 전부에게 적어 준다 —
    // 카드마다 다시 걷으면 카드 수의 제곱이 된다.
    const seen = new Set<string>([i.id]);
    const stack = [i.id];
    while (stack.length) {
      const cur = stack.pop()!;
      for (const n of near.get(cur) ?? []) {
...
/** 낱개 질의용 — 위와 같은 규칙이다. */
export function connectedGroup(items: readonly TreeItem[], id: string): string[] {
  const near = new Map<string, string[]>();
  const add = (a: string, b: string) => {
    const cur = near.get(a);
    if (cur) cur.push(b);
