...
describe("buildInkScene — 표시 읽기", () => {
  ...
  /**
   * **겨눈 카드가 반드시 들어간다** (계약이 바뀌었다 — 사용자 지시 2026-08-12).
   *
   * 예전에는 "옆으로 비껴 있는 카드는 절대 안 집는다"였다. 그 말 자체는 지금도
   * 맞지만, 관용도를 그만큼 좁게 잡으면 **화살표가 카드에 닿지 않는 한 아무
   * 카드도 안 잡혀** 학생이 되물음만 받는다(사용자 보고). 이제 곁의 카드가
   * 딸려 오는 것은 허용하고, **겨눈 카드가 확신 순에서 앞선다**는 것으로
   * 규칙을 옮긴다 — 이어 묻기의 부모가 그 순서로 정해지기 때문이다.
   */
  it("겨눈 카드가 확신 순에서 앞선다 (곁의 카드가 딸려 와도)", () => {
    const cards = [
