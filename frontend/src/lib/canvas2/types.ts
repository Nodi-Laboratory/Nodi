...
export interface ItemData {
   ...
   */
  /**
   * 딸린 상자(도판·클립)를 **접어 뒀나** (사용자 지시 2026-08-12).
   *
   * 접으면 캔버스에서 사라지고 부모 카드 안의 **동그란 단추**가 된다. 그
   * 단추를 누르면 다시 펼쳐진다.
   *
   * `data`에 두는 이유는 코치 말풍선과 같다 — **새로고침해도 남아야 한다.**
   * 접어 둔 것이 방을 다시 열 때마다 펼쳐지면 접는 뜻이 없다.
   *
   * ⚠️ 지우는 것과 다르다. 접은 것은 그 자리에 그대로 있고(단추가 그 증거다)
   * 지운 것은 없어진다 — 학생이 잘못 접었을 때 되돌릴 길이 있어야 한다.
   */
  collapsed?: boolean;
  coach?: {
    advice: {
