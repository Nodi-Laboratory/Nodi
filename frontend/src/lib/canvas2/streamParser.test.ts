...
describe("머리표를 빠뜨린 답 되살리기 (2026-08-07 실측)", () => {
  ...
  /**
   * 사용자 보고 2026-08-12: "질문했는데 응답이 안 생겨" — 자판·질문펜 둘 다.
   * 서버 턴 로그에는 `missing_concept_envelope`만 남아 있었다.
   *
   * 모델이 머리표를 빠뜨리는 것까지는 위 그물이 잡는데, **빠뜨린 줄들이
   * `CHAT:`을 달고 오면** 전부 말풍선으로 흘러 `orphan`이 빈 채 끝났다.
   * 되살릴 것이 없으니 카드가 안 생긴다 — 답은 왔는데 캔버스는 그대로다.
   */
  const 말풍선으로만_온_답 =
    "CHAT: 광합성은 식물이 **빛에너지**를 화학에너지로 바꾸는 과정이에요.\n" +
    "CHAT: 잎의 엽록체에서 일어나고, 이산화탄소와 물을 재료로 씁니다.\n" +
    "CHAT: 그 결과 포도당과 산소가 만들어져요. 우리가 숨 쉬는 산소의 대부분이 " +
    "여기서 나온다고 보면 됩니다.";

  it("긴 설명이 말풍선으로만 와도 카드가 생긴다", () => {
    const { concepts, reply } = run(말풍선으로만_온_답);
    expect(concepts).toHaveLength(1);
    expect(concepts[0].body).toContain("광합성은 식물이");
    expect(concepts[0].body).toContain("포도당과 산소");
    // 제목은 첫 굵은 낱말에서 빌린다(머리표 없는 답과 같은 규칙).
    expect(concepts[0].title).toBe("빛에너지");
    // ⚠️ 말풍선은 **그대로 뜬다.** 카드로 살렸다고 가로채면, 정상 턴에서
    // 말풍선이 사라지는 부작용이 생긴다.
    expect(reply).toContain("포도당과 산소");
  });

  it("자료만 추천한 턴의 안내 한 줄은 카드가 안 된다", () => {
    // D210 7-1: 설명 없이 자료만 주는 턴은 **의도된** 말풍선이다.
    const { concepts } = run(
      "CHAT: 요청하신 그림을 찾아 옆에 띄웠어요. 궁금한 게 더 있으면 말해 주세요.",
    );
    expect(concepts).toEqual([]);
  });

  it("카드가 하나라도 있으면 말풍선은 살리지 않는다", () => {
    // 정상 턴의 말풍선까지 카드로 만들면 한 턴에 카드가 둘 생긴다(D162).
    const { concepts } = run(
      "CHAT: 좋은 질문이에요! 아래에 정리해 둘게요. 이 문장은 백육십 자를 넘기려고 " +
        "일부러 길게 씁니다. 이렇게 길어도 카드가 이미 있으면 살리지 않아야 합니다. " +
        "그래야 한 턴에 카드 하나라는 규칙이 지켜집니다.\n" +
        "@concept: 광합성 | 생명과학\n본문입니다.\n@end",
    );
    expect(concepts).toHaveLength(1);
    expect(concepts[0].title).toBe("광합성");
  });
