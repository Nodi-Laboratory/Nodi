        ...
        await streamChat(
          ...
          },
          {
            ...
            onDone: (d: ChatDoneEvent) => {
              ...
              // 필요할 때 재발급한다.
              /**
               * 몇 장을 붙일지는 **관리자 노브가 정한다**
               * (`figure_retrieve_top_k`, 기본 1 · 상한 5, 사용자 지시 2026-08-12).
               *
               * 한때 여기서 1로 잘랐다. 그러면 관리자가 값을 올려도 화면이
               * 안 따라와서, **콘솔이 거짓말을 하는** 상태가 된다. 서버가 이미
               * 그 수만큼만 돌려주므로 화면은 받은 것을 그대로 그리면 된다 —
               * 자르는 자리가 둘이면 반드시 갈린다.
               */
