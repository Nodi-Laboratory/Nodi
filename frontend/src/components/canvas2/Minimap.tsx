 ...
 * 읽힌다. 이 지도에는 본문 의미쌍(`--c-live` AI 초록 · `--c-hand` 학생 밤색)이
 * 같은 화면에 함께 뜨므로 **그 두 대역을 비워 둔다**(D203).
 *
 * 옛 6색에는 `#b4692a`(색상 27°)와 `#4d7a3a`(102°)가 있었다 — 초록 전환 뒤엔
 * 각각 학생 밤색(25°)·AI 초록(85°)과 같은 대역이라 "트리 색인지 의미색인지"가
 * 흐려진다. 지금 6색은 168·204·234·274·312·350°로, 초록에서 83° 떨어져 있다.
 *
 * ⚠️ **고를 때 상호 대비(WCAG)로 재지 마라.** 명도가 비슷하면 눈에 뚜렷이
 * 갈리는 색끼리도 1.0 근처가 나온다 — 그 잣대로 고르면 명도만 벌어진 색
 * 여섯이 되어 오히려 못 갈린다. 여기서 재야 할 것은 **색상 간격**(최소 30°)
 * 이고, 종이 대비는 각 색마다 따로 본다(전부 4.8~5.6).
 */
const TREE_COLORS = [
  "#2f7d6e",
  "#2a76a8",
