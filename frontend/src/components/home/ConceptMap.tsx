 ...
 * 배경이 아니라 방해다 — 있는 줄 알겠는 정도가 맞다.
 *
 * ⚠️ **화면이 넓어지면 같은 값이 세진다** (2026-08-11). 상자를 걷어내 지도가
 * 화면 전체를 쓰게 되자 `FIT_BOOST`가 배율을 올렸고, 세기는 그대로인데
 * 움직임이 세 보였다("심하다"). 세기는 **월드 단위**이고 눈에 보이는 것은
 * 배율이 곱해진 뒤라, 화면 크기를 바꾸면 이 값도 다시 재야 한다.
 *
 * 실측(250ms 사이 변한 픽셀, 12회 평균):
 *   상자 있던 때  0.3 →  4.5%  ·  1.05 → 9.6%
 *   상자 걷은 뒤  0.3 → 23.2%  ·  0.08 → 약 5%
 *
 * ⚠️ 이 자로는 **0.12 이상을 구별 못 한다**(0.12도 0.3도 22~23%). 노드가 제
 * 반지름보다 크게 움직이면 그 픽셀은 어차피 다 바뀌어 지표가 포화한다 —
 * 값을 고를 때는 안 포화된 구간(0.1 아래)에서 재야 한다.
 *
